...
function App() {
  ...
  const getQuestions = async () => {
    const api = await fetch(`https://opentdb.com/api.php?amount=10${searchParams.category && `&category=${searchParams.category}`}&difficulty=${searchParams.difficulty.toLowerCase()}&type=multiple`)
    if(!api.ok) {
  ...
  return (
    <div className="h-screen sm:h-full ipad:h-screen lg:screen box-border relative font-quicksand bg-gray-800 bg-fixed">
      <Header />
      <Routes>
        <Route path="/" element={<Home handleChange={handleChange} {...searchParams} handleName={handleName} name={name} handleSubmit={handleSubmit} setQuestions={setQuestions} setScore={setScore} /> } />
