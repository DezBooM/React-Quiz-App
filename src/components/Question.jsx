...
function Question({
  ...
  return (
    <div className="mt-4 md:mt-10 text-cyan-300 flex flex-col items-center">
      ...
      <h1 className="text-2xl md:text-4xl mx-2 text-center tracking-tight leading-tight my-5">
      ...
      <div className="mt-4 flex flex-col w-11/12 md:w-4/6 md:grid md:grid-cols-2 gap-4 md:gap-10">
        {options &&
          options.map((option) => (
            <button
              className={`px-10 py-3 ${style}  text-gray-800 tracking-tight leading-tight shadow-custom active:shadow-customInner rounded-lg font-semibold text-xl ${
                selected && handleSelect(option, style)
              }`}
              key={option}
              disabled={selected}
